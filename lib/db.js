...
// Every progress row across every account, for building the admin's
// per-user accountability grids in one query instead of N.
export async function adminGetAllProgress(supabase) {
  const { data, error } = await supabase
    .from("user_progress")
    .select("user_id, day, done");
  if (error) throw error;
  return data || [];
}
